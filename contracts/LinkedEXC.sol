 ...
 contract LinkedEXC {
    ...
    */
    function addSellOrder(uint amount, uint raterequest) payable public returns (bool success) {
        ...
        matchingOrdersSell(amount, costWei, msg.sender);
    ...
    */
    function matchingOrdersBuy(uint amount, uint costWei) internal returns (bool success) {
        require(token.balanceOf(this) >= costWei);
        assert(token.transfer(msg.sender, amount));
        return true;
    }
    function matchingOrdersSell(uint amount, uint costWei, address seller) internal returns (bool success) {
