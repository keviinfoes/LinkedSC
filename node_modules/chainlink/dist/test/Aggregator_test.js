...
contract('Aggregator', () => {
    ...
    describe('#requestRateUpdate', () => {
        ...
        context('basic updates', () => {
            beforeEach(async () => {
                rate = await Aggregator.new(link.address, basePayment, 1, [oc1.address], [jobId1]);
                await link.transfer(rate.address, deposit);
                const current = await rate.currentAnswer.call();
                matchers_1.assertBigNum(h.bigNum(0), current);
            });
            it('trigger a request to the oracle and accepts a response', async () => {
                ...
                const current = await rate.currentAnswer.call();
                matchers_1.assertBigNum(response, current);
            });
            ...
            it('emits a log with the response, answer ID, and sender', async () => {
                ...
                assert.equal(oc1.address, web3.utils.toChecksumAddress(receivedLog.topics[3].slice(26, 66)));
            });
            it('emits a log with the new answer', async () => {
                ...
                assert.equal(response, web3.utils.hexToNumber(answerUpdatedLog.topics[1]));
                assert.equal(answerId, web3.utils.hexToNumber(answerUpdatedLog.topics[2]));
            });
        });
        context('with multiple oracles', () => {
            beforeEach(async () => {
                rate = await Aggregator.new(link.address, basePayment, oracles.length, oracles.map(o => o.address), [jobId1, jobId2, jobId3]);
                await link.transfer(rate.address, deposit);
                const current = await rate.currentAnswer.call();
                matchers_1.assertBigNum(h.bigNum(0), current);
            });
            it('triggers requests to the oracles and the median of the responses', async () => {
                ...
                const current = await rate.currentAnswer.call();
                matchers_1.assertBigNum(77, current);
            });
            it('does not accept old responses', async () => {
                ...
                matchers_1.assertBigNum(response2, await rate.currentAnswer.call());
                for (let i = 0; i < oracles.length; i++) {
        ...
        context('with an even number of oracles', () => {
            beforeEach(async () => {
                ...
                rate = await Aggregator.new(link.address, basePayment, oracles.length, oracles.map(o => o.address), [jobId1, jobId2, jobId3, jobId4]);
                await link.transfer(rate.address, deposit);
                const current = await rate.currentAnswer.call();
                matchers_1.assertBigNum(h.bigNum(0), current);
            });
    ...
    describe('#updateRequestDetails', () => {
        ...
        context('when called by the owner', () => {
            it('changes the amout of LINK sent on a request', async () => {
                ...
                await rate.updateRequestDetails(uniquePayment, 1, [oc2.address], [jobId2], {
                    from: personas.Carol,
                });
                await rate.requestRateUpdate({ from: personas.Carol });
                matchers_1.assertBigNum(uniquePayment, await link.balanceOf.call(oc2.address));
            });
            it('can be configured to accept fewer responses than oracles', async () => {
                await rate.updateRequestDetails(basePayment, 1, [oc1.address, oc2.address], [jobId1, jobId2], {
                    from: personas.Carol,
                });
                ...
                matchers_1.assertBigNum(response1, await rate.currentAnswer.call());
                const response2 = 200;
                await h.fulfillOracleRequest(oc2, request2, response2);
                matchers_1.assertBigNum((response1 + response2) / 2, await rate.currentAnswer.call());
            });
            context('and the number of jobs does not match number of oracles', () => {
                it('fails', async () => {
                    await h.assertActionThrows(async () => {
                        await rate.updateRequestDetails(basePayment, 2, [oc1.address, oc2.address], [jobId2], {
                            from: personas.Carol,
                        });
                    });
                });
            ...
            context('and the oracles required exceeds the available amount', () => {
                it('fails', async () => {
                    await h.assertActionThrows(async () => {
                        await rate.updateRequestDetails(basePayment, 3, [oc1.address, oc2.address], [jobId1, jobId2], {
                            from: personas.Carol,
                        });
                    });
                });
        ...
        context('when called by a non-owner', () => {
            it('fails', async () => {
                await h.assertActionThrows(async () => {
                    await rate.updateRequestDetails(basePayment, 1, [oc2.address], [jobId2], {
                        from: personas.Eddy,
                    });
                });
            });
        ...
        context('when called before a past answer is fulfilled', () => {
            ...
            it('accepts answers from oracles at the time the request was made', async () => {
                ...
                matchers_1.assertBigNum(212, await rate.currentAnswer.call());
            });
        });
        context('when calling with a large number of oracles', () => {
            ...
            it(`does not revert with up to ${maxOracleCount} oracles`, async () => {
                ...
                await rate.updateRequestDetails(basePayment, maxOracleCount, oracles, jobIds, { from: personas.Carol });
            });
            it(`reverts with more than ${maxOracleCount} oracles`, async () => {
                ...
                await h.assertActionThrows(async () => {
                    await rate.updateRequestDetails(basePayment, overMaxOracles, oracles, jobIds, { from: personas.Carol });
                });
            });
        });
    });
    describe('#transferLINK', () => {
        beforeEach(async () => {
            ...
            await rate.transferOwnership(personas.Carol);
            await link.transfer(rate.address, deposit);
            matchers_1.assertBigNum(deposit, await link.balanceOf.call(rate.address));
        });
        context('when called by the owner', () => {
            it('succeeds', async () => {
                ...
                matchers_1.assertBigNum(0, await link.balanceOf.call(rate.address));
    ...
    describe('#cancelRequest', () => {
        ...
        beforeEach(async () => {
            ...
            matchers_1.assertBigNum(basePayment, await link.balanceOf.call(rate.address));
            matchers_1.assertBigNum(0, await link.balanceOf.call(oc1.address));
            const requestTx = await rate.requestRateUpdate();
            request = h.decodeRunRequest(requestTx.receipt.rawLogs[3]);
            matchers_1.assertBigNum(0, await link.balanceOf.call(rate.address));
            matchers_1.assertBigNum(basePayment, await link.balanceOf.call(oc1.address));
        ...
        context('when a later answer has been provided', () => {
            beforeEach(async () => {
                ...
                matchers_1.assertBigNum(basePayment * 2, await link.balanceOf.call(oc1.address));
            });
    ...
    context('testing various sets of inputs', () => {
        ...
        for (const test of tests) {
            ...
            it(test.name, async () => {
                ...
                matchers_1.assertBigNum(test.want, await rate.currentAnswer.call());
            });
        }
    });
});
//# sourceMappingURL=Aggregator_test.js.map