...
contract('AggregatorProxy', () => {
    ...
    beforeEach(async () => {
        ...
        aggregator = await Aggregator.new(link.address, basePayment, 1, [oc1.address], [jobId1]);
    ...
    describe('#currentAnswer', () => {
        beforeEach(async () => {
            ...
            matchers_1.assertBigNum(response, await aggregator.currentAnswer.call());
        });
        it('pulls the rate from the aggregator', async () => {
            matchers_1.assertBigNum(response, await proxy.currentAnswer.call());
        });
        context('after being updated to another contract', () => {
            beforeEach(async () => {
                aggregator2 = await Aggregator.new(link.address, basePayment, 1, [oc1.address], [jobId1]);
                ...
                matchers_1.assertBigNum(response2, await aggregator2.currentAnswer.call());
                await proxy.setAggregator(aggregator2.address);
            });
            it('pulls the rate from the new aggregator', async () => {
                matchers_1.assertBigNum(response2, await proxy.currentAnswer.call());
            });
        });
    });
    describe('#updatedHeight', () => {
        ...
        it('pulls the height from the aggregator', async () => {
            matchers_1.assertBigNum(await aggregator.updatedHeight.call(), await proxy.updatedHeight.call());
        });
        context('after being updated to another contract', () => {
            beforeEach(async () => {
                aggregator2 = await Aggregator.new(link.address, basePayment, 1, [oc1.address], [jobId1]);
            ...
            it('pulls the height from the new aggregator', async () => {
                matchers_1.assertBigNum(await aggregator2.updatedHeight.call(), await proxy.updatedHeight.call());
            });
        });
    });
...
//# sourceMappingURL=AggregatorProxy_test.js.map