...
const GetterSetterFactory_1 = require("contracts/GetterSetterFactory");
const EmptyOracleFactory_1 = require("contracts/EmptyOracleFactory");
const OracleFactory_1 = require("contracts/OracleFactory");
const ConcreteChainlinkedFactory_1 = require("contracts/ConcreteChainlinkedFactory");
...
describe('ConcreteChainlinked', () => {
    ...
    describe('#cancelChainlinkRequest', () => {
        ...
        it('emits an event from the contract showing the run was cancelled', async () => {
            const tx = await cc.publicCancelRequest(requestId, 0, ethers_1.ethers.utils.hexZeroPad('0x', 4), 0);
            const { events } = await tx.wait();
        ...
        it('throws if given a bogus event ID', async () => {
            await h.assertActionThrows(async () => {
                await cc.publicCancelRequest(ethers_1.ethers.utils.formatBytes32String('bogusId'), 0, ethers_1.ethers.utils.hexZeroPad('0x', 4), 0);
            });
        });
    });
